## Etapa 5 — Especificar a primeira fatia vertical

Agora vamos transformar o cenário em um contrato versionado.

Use como insumo:

- as ambiguidades levantadas;
- a comparação entre zero-shot e few-shot;
- a discussão em sala.

Em grupo, rascunhe a especificação da primeira fatia vertical e salve o resultado em:

```text
docs/specs/training-catalog-vertical-slice.md
```

O arquivo deve registrar pelo menos:

1. objetivo da fatia;
2. escopo e fora de escopo;
3. dados do treinamento;
4. contrato do `POST /api/trainings`;
5. comportamento esperado da interface;
6. critérios de aceitação;
7. evidências esperadas;
8. decisões ainda abertas.

Se quiser comparar o seu rascunho com o material ensaiado, use este conteúdo como referência amigável:
